...
```mermaid
flowchart LR
    A[Data: Generation] --> B(Data: Ingest) --> C(Data: Transform) --> D(Data: Load) --> E(Data: Explore) --> F(Data: Explore) -->|Move Code to Source Code Package| O(SE: Integrate) & P(DS/SE: Microservice) --> (IT: Deploy)
